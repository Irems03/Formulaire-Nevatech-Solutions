...
export function validateClientInfo(info: ClientInfo): Record<string, string> {
  const errors: Record<string, string> = {};

  if (!info.contactName.trim()) {
    errors.contactName = "Le nom du contact est requis.";
  } else if (info.contactName.trim().length < 2) {
    errors.contactName = "Le nom doit comporter au moins 2 caractères.";
  }
  ...
  if (!info.sector.trim()) {
    errors.sector = "Veuillez sélectionner le secteur d'activité de votre entreprise.";
  }

  if (!info.contactRole.trim()) {
    errors.contactRole = "Le rôle du contact est requis.";
  }

  if (!info.projectObjective.trim()) {
    errors.projectObjective = "L'objectif principal du projet est requis.";
  }
