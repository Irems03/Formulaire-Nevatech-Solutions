...
import { FormSubmission, calculateDevEstimate, PricingSettings, ErrorLog, AnalyticsStats, GeneralSettings } from './types';
 ...
 */
export async function submitSupportMessage(name: string, email: string, message: string): Promise<{ success: boolean; error?: string }> {
  const supabase = getSupabase();
